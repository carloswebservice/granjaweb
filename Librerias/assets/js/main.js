	...
	var runQuickSideBar = function() {
		$(".sb-toggle").on("click", function(e) {
			if($(this).hasClass("open")) {
				...
				$("#servicio").hide(); $("#aborto").hide(); $("#muerte").hide(); $("#venta").hide(); $("#celo").hide();
			    $("#parto").hide(); $("#secado").hide();$("#rechazo").hide();$("#medicacion").hide();$("#indicaciones").hide();
			    $("#enfermedad").hide();$("#analisis").hide(); $("#tactorectal").hide();
			}
	...
	var runSaveSetting = function() {
		$('.save_style').on('click', function() {
			...
			el.block({
				...
				message: '<img src="assets/images/loading.gif" /> Actualizando Informacion...',
				css: {
					border: 'none',
					color: '#333',
					background: 'none'
