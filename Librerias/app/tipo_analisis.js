$(function(){
	...
	$("#reqtipoanalisis").addClass("active");
})
...
function guardar(campo){
	...
	$.ajax({
		type:"POST",
		data:$('#form_tipo_analisis').serialize(),
		url:URL+'tipo_analisis/guardar',
...
function modifitpa(id){
	$.ajax({
		type:"POST",
		data:"id="+id,
		url:URL+'tipo_analisis/modificar',
		success: function(data){
			var datos = eval(data);
...
function eliminar(id){
	...
	if (window.confirm("SEGURO QUE QUIERES ELIMINAR ?")) {
		$.ajax({
			type:"POST",
			data:"id="+id,
			url:URL+'tipo_analisis/eliminar',
			success: function(data){
...
function actualizar() {
	location.href=URL+'tipo_analisis';
}

function nuevocancel(){
	$("#form_tipo_analisis").trigger("reset");
